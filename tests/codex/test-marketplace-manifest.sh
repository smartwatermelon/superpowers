...
# This fork has no hooks/hooks.json (the SessionStart hook was removed), so
# there's nothing for Codex's hooks/hooks.json auto-discovery fallback to
# find. Declaring an empty inline hooks object ({}) is harmless either way.
assert_equal(
    manifest.get("hooks"),
    {},
    "Codex manifest declares empty hooks {}",
)
